...
export default class Emboss implements ImageFilterRoute {
    ...
    public constructor(image_data: Buffer, query: any) {
        ...
        this.query = query;
        Bounds.check("emboss", this.query);
    }
    
    public async apply(): Promise<Buffer> {
        return magick(this.image_data, [`-colorspace`, `Gray`, `-emboss`, `0x${this.query.embossity ?? 0.1}`]);
    }
